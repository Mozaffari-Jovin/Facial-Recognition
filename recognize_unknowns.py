...
for fn in file_names:
	...
	for (loc, encod) in zip(locs, encods):
		...
		results = face_recognition.compare_faces(encodings, encod, 0.5) # compare encod with encodings
		...
		for (i, res) in enumerate(results):
			if res:
				new_name = names[i]
				cv2.putText(img, new_name, top_left, cv2.FONT_HERSHEY_COMPLEX, 0.5, (255, 255, 255), 1)
				break
